...
public class ActionController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        PropSearch();       // F키(다른 스크립트에 있음) - 물체와 상호작용하는 함수
    ...
    private void PropSearch()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hitinfo2, range, layerMask2))
        {
            Debug.Log("인식됨2222");
            masking = hitinfo2.transform.GetComponent<Outline>();
    ...
    private void CheckItem()
    {
        if (Physics.Raycast(transform.position, transform.forward, 
                out hitinfo1, range, layerMask1)) //광선쏘기(.., 충돌체정보, 사정거리, 레이어마스크)
        { //레이캐스트에 닿으면
            Debug.Log("인식됨");
            if (hitinfo1.transform.tag == "getItem") //레이캐스트에 닿은 물체의 태그가 다음과 같을 시
            {
                pickupActivated = true; //습득 가능한 상태가 되고
                masking = hitinfo1.transform.GetComponent<Outline>();
                masking.enabled = true;
            }
        }
        else//레이캐스트를 벗어나면
    ...
    private void PickupItem()
    {
        ...
        child.transform.localPosition = new Vector3(0, 1, 1);   //위치설정
        ...
        if (hitinfo1.transform.tag == "getItem")
        {
            ...
            if (child.name == "Flash") // 후레시를 들었을 경우
            {
                if (child.transform.childCount > 0)
                {
                    ...
                    child.transform.localRotation = Quaternion.Euler(0, -70, -100); //방향 설정
                }
            }
        }
    }
    ...
    public void pickupItemFromInventory(int id) //Inventory.cs에서 사용, 인벤토리에 아이템 넣는 함수
    {
        ...
        child.transform.localPosition = new Vector3(0, 1, 1);
        got = true;
